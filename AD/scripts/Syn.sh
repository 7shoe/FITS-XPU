...
for win_size in ${win_sizes[@]}; do
    for batch_size in ${batch_sizes[@]}; do
        for lr in ${lrs[@]}; do
            python main.py --anormly_ratio 5 --num_epochs 50    --batch_size $batch_size  --mode train --dataset Syn  --data_path dataset/Syn --input_c 5    --output_c 5  --win_size $win_size --DSR $dsr --lr $lr --cutfreq $cutfreq >logs/ft_Syn/training_bs${batch_size}_win${win_size}_ds${dsr}_lr${lr}_c${cutfreq}.log

            for ar in {5..25}; do
                python main.py --anormly_ratio $ar  --num_epochs 50       --batch_size $batch_size     --mode test    --dataset Syn   --data_path dataset/Syn  --input_c 5    --output_c 5  --pretrained_model 20  --win_size $win_size --DSR $dsr --lr $lr --cutfreq $cutfreq >logs/ft_Syn/testing_ar${ar}_bs${batch_size}_win${win_size}_ds${dsr}_lr${lr}_c${cutfreq}.log
            done
...
# python main.py --anormly_ratio 5 --num_epochs 50    --batch_size 1  --mode train --dataset SWAT  --data_path dataset/Syn --input_c 55    --output_c 55  --win_size 200 --DSR 4 --lr 1 --cutfreq 0