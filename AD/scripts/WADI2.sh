...
if [ ! -d "./logs/ft_WADI_ind" ]; then
    mkdir ./logs/ft_WADI_ind
fi

dsr=4
win_sizes=(400)
batch_sizes=(512) # (32 64 128 256)
lrs=(5e-4)
cutfreq=0
...
for win_size in ${win_sizes[@]}; do
    for batch_size in ${batch_sizes[@]}; do
        for lr in ${lrs[@]}; do
            python main.py --anormly_ratio 5 --num_epochs 50    --batch_size $batch_size  --mode train --dataset WADI  --data_path dataset/WADI --input_c 55    --output_c 55  --win_size $win_size --DSR $dsr --lr $lr --cutfreq $cutfreq --model_save_path ./checkpoint/${batch_size}_win${win_size}_ds${dsr}_lr${lr}_c${cutfreq}_ind --individual True >logs/ft_WADI_ind/training_bs${batch_size}_win${win_size}_ds${dsr}_lr${lr}_c${cutfreq}.log
